    ...
    * This allows for more accurate representation of pump performance under varying operating conditions.
    ...
    CONFIG.declare(
        ...
        ConfigValue(
            ...
            description="Head and efficiency curve at reference speed",
...
# Replace the DeltaP equation from the base pump model with one which includes geometric head
        ...
        if self.config.variable_efficiency is not Efficiency.Fixed:
            ...
            self.design_efficiency = Var(
                ...
                This could be the efficiency at the best efficiency point (BEP) or user selected operating point.""",
            ...
            self.efficiency_surrogate_coefficients = Param(
                self.surrogate_index,
                initialize=efficiency_surrogate_coeffs,
            ...
            def ref_head_surrogate(b):
                ...
                return (
                    ...
                    == (
                        ...
                        + b.head_surrogate_coefficients[1]
                        * (b.ref_flow / (pyunits.m**3 / pyunits.s))
                        + b.head_surrogate_coefficients[2]
                        * ((b.ref_flow / (pyunits.m**3 / pyunits.s)) ** 2)
                        + b.head_surrogate_coefficients[3]
                        * ((b.ref_flow / (pyunits.m**3 / pyunits.s)) ** 3)
                    )
                    * pyunits.m
                )
            ...
            def ref_efficiency_constraint(b):
                ...
                return b.ref_efficiency == (
                    ...
                    + b.efficiency_surrogate_coefficients[1]
                    * (b.ref_flow / (pyunits.m**3 / pyunits.s))
                    + b.efficiency_surrogate_coefficients[2]
                    * ((b.ref_flow / (pyunits.m**3 / pyunits.s)) ** 2)
                    + b.efficiency_surrogate_coefficients[3]
                    * ((b.ref_flow / (pyunits.m**3 / pyunits.s)) ** 3)
                )
