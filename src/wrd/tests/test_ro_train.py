...
def test_ro_train1_8_19_21():
    ...
    actual_power = pyunits.convert(
        m.fs.ro_train.total_pump_power, to_units=pyunits.kW
    )
    assert value(actual_power) == pytest.approx(value(expected_power), rel=0.15)

    actual_product_flow = pyunits.convert(
        m.fs.ro_train.product.properties[0].flow_vol_phase["Liq"], to_units=pyunits.gal / pyunits.min
    )
    assert value(actual_product_flow) == pytest.approx(value(expected_product_flow), rel=0.15)
...
def test_ro_train1_3_13_21():
    ...
    m = main(Qin=2452, Cin=0.503, Tin=295, Pin=101325, file="wrd_ro_inputs_3_13_21.yaml")
    actual_power = pyunits.convert(
        m.fs.ro_train.total_pump_power, to_units=pyunits.kW
    )
    assert value(actual_power) == pytest.approx(value(expected_power), rel=0.15)
